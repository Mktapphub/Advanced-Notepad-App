...
public class AddNoteActivity extends  AppCompatActivity  { 
	...
	private void initialize(Bundle _savedInstanceState) {
			...
			public void onClick(View _view) {
				try{
					if (title_edittext.getText().toString().equals("") && note_edittext.getText().toString().equals("")) {
						...
						if (title_edittext.getText().toString().equals("") && !note_edittext.getText().toString().equals("")) {
							SketchwareUtil.showMessage(getApplicationContext(), "Enter Note Title !!");
						}
						else {
							if (note_edittext.getText().toString().equals("") && !title_edittext.getText().toString().equals("")) {
								SketchwareUtil.showMessage(getApplicationContext(), "Enter Your Note !!");
							}
							else {
								AddNote = new HashMap<>();
		...
		paste.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				android.content.ClipboardManager paste = (android.content.ClipboardManager) getSystemService(CLIPBOARD_SERVICE);
				note_edittext.setText(note_edittext.getText().toString().concat(paste.getText().toString()));
				int p = note_edittext.length();
				note_edittext.setSelection(p);
			}
		});
		...
		note_edittext.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence _param1, int _param2, int _param3, int _param4) {
				...
				if (!(9 < _charSeq.length())) {
					title_edittext.setText(note_edittext.getText().toString());
				}
				else {
					if (9 < _charSeq.length()) {
	...
	public void _ONCREATE () {
		importantcheckbox.setVisibility(View.GONE);
					...
					if (Settings.getString("Theme", "").equals("Orange")) {
						...
						if (Settings.getString("Theme", "").equals("Indigo")) {
							...
							if (Settings.getString("Theme", "").equals("Pink")) {
								...
								toolbar.setBackgroundColor(0xFFFFFFFF);
								main.setBackgroundColor(0xFFFFFFFF);
								title_edittext.setTextColor(0xFFE91E63);
								title_edittext.setHintTextColor(0xFFE91E63);
								note_edittext.setTextColor(0xFF000000);
								note_edittext.setHintTextColor(0xFF000000);
								importantcheckbox.setTextColor(0xFFE91E63);
								importantcheckbox.getButtonDrawable().setColorFilter(Color.parseColor("#FF5722"), PorterDuff.Mode.SRC_IN);
