...
public class NotesListActivity extends  AppCompatActivity  { 
	...
	private double search_position = 0;
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		notes_list.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
			...
			public boolean onItemLongClick(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				final int _position = _param3;
				if (Selected) {
					SketchwareUtil.showMessage(getApplicationContext(), "Selection Mode Is Enabled");
				}
				else {
					Selected = true;
					Notes_List.get((int)(Notes_List.size() - 1) - _position).put("Select", "True");
					...
					if (Settings.getString("Theme", "").equals("Orange")) {
						...
						if (Settings.getString("Theme", "").equals("Indigo")) {
							...
							if (Settings.getString("Theme", "").equals("Pink")) {
								...
								main.setBackgroundColor(0xFFFFFFFF);
								_fab.setBackgroundTintList(android.content.res.ColorStateList.valueOf(Color.parseColor("#E91E63")));
								_Add("#E91E63", back_img);
								_Add("#E91E63", select_all_img);
							}
							else {
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		androidx.appcompat.widget.SearchView mSearchView = new androidx.appcompat.widget.SearchView(getSupportActionBar().getThemedContext());
		LinearLayout.LayoutParams viewSearch = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
		viewSearch.weight = 0;
		mSearchView.setLayoutParams(viewSearch);
		...
		mSearchView.setOnQueryTextListener(new androidx.appcompat.widget.SearchView.OnQueryTextListener() {
			 ...
			 @Override public boolean onQueryTextChange(String newText) {
				...
				if (TextUtils.isEmpty(newText)) {
					...
					_saerch_list(charSeq);
				} else {
					_saerch_list(charSeq);
				}
				return true;
			}});
		menu.add(0, 0, 0, "Search").setIcon(R.drawable.ic_search).setActionView(mSearchView).setShowAsAction(/*MenuItem.SHOW_AS_ACTION_IF_ROOM | MenuItem.SHOW_AS_ACTION_COLLAPSE_ACTION_VIEW*/10);
		return true;
	}
	...
	public void _saerch_list (final String _charSeq) {
		if (!AllNotes.getString("Notes", "").equals("")) {
			...
			if (_charSeq.length() > 0) {
				search_n1 = (Notes_List.size() - 1) - search_position;
				search_n = Notes_List.size();
				for(int _repeat91 = 0; _repeat91 < (int)(search_n1); _repeat91++) {
					if (Notes_List.get((int)search_n1).get("Title").toString().toLowerCase().contains(_charSeq.toLowerCase())) {
						
					}
					else {
						Notes_List.remove((int)(search_n1));
					}
					search_n1--;
				}
			}
	...
	public class Notes_listAdapter extends BaseAdapter {
		...
		public View getView(final int _position, View _v, ViewGroup _container) {
			...
			if (Settings.getString("Theme", "").equals("Default")) {
				_SX_CornerRadius_4(side_lin, "#FF009688", "#FF009688", 0, 30, 30, 30, 30);
				_SetRadiusToView(main, 30, "#F6F7FB");
				_SetShadowOfView(main, 5);
				_Add("#FF009688", imageview1);
			}
			else {
				if (Settings.getString("Theme", "").equals("Dark")) {
					_SX_CornerRadius_4(side_lin, "#FF009688", "#FF009688", 0, 30, 30, 30, 30);
					...
					if (Settings.getString("Theme", "").equals("Blue Grey")) {
						...
						if (Settings.getString("Theme", "").equals("Orange")) {
							...
							if (Settings.getString("Theme", "").equals("Indigo")) {
								...
								if (Settings.getString("Theme", "").equals("Pink")) {
									_SX_CornerRadius_4(side_lin, "#E91E63", "#E91E63", 0, 30, 30, 30, 30);
									_SetRadiusToView(main, 30, "#FFFFFF");
									_SetShadowOfView(main, 5);
									title.setTextColor(0xFFE91E63);
									note.setTextColor(0xFF000000);
									_Add("#E91E63", imageview1);
								}
								else {
